...
export function hasIridiumMessagePrefix(encoded: Uint8Array): boolean {
  ...
  return encoding.arrayToHex(sliced) === encoding.arrayToHex(IRIDIUM_MESSAGE_PREFIX);
}
...
export function encodeIridiumV1Message(msg: string, opts?: IridiumV1MessageOptions) {
  ...
  const length = new Uint8Array(IRIDIUM_HEADER?.["1"].LENGTH_SIZE);
  // Little endian
  ...
  const rawArray = encoding.concatArrays(prefix, version, length, message, prompt);
